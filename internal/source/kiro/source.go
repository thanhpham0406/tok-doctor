...
func (s *Source) Detect(ctx context.Context, override source.Override) source.Detection {
	_ = ctx
	if override.Path != "" {
		return source.DetectConfiguredInstallPath(s.Name(), s.DisplayName(), override.Path, override.Origin, "source path exists but supported Kiro data detection is not implemented yet")
	}
